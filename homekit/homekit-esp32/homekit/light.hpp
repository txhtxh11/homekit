...
namespace esphome
{
  namespace homekit
  {
    // ==== 真正的骨科手术：手写监听器类，专门用来兼容 2026.1+ 新版 ESPHome API ====
    class HAPLightListener : public light::LightTargetStateReachedListener {
     public:
      HAPLightListener(std::function<void()> cb) : cb_(cb) {}
    ...
    public:
      ...
      void setup() {
        ...
        if (!lightPtr->is_internal()) {
          // ==== 我们自己注入的代码，完美修复 ====
          auto *listener = new HAPLightListener([this]() { LightEntity::on_light_update(this->lightPtr); });
